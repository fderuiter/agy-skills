---
layout: default
title: "Token | AI Coding Dictionary"
...
permalink: /dictionary/token/
category: dictionary
keywords: ["ai coding", "dictionary", "token"]
---

# Token

The atomic unit a [model](https://fderuiter.github.io/agy-skills/dictionary/model) reads and writes. Roughly word-sized but not exactly, common words are one token, rare or long ones split into several. [Context window](https://fderuiter.github.io/agy-skills/dictionary/context-window) size, cost, and latency are all counted in tokens.

Text becomes tokens via a tokenizer: a fixed vocabulary of tens of thousands of fragments, learned before [training](https://fderuiter.github.io/agy-skills/dictionary/training), that splits any input into a sequence of vocabulary entries. The model never sees characters or words, every piece of text is converted to tokens on the way in, and [next-token prediction](https://fderuiter.github.io/agy-skills/dictionary/next-token-prediction) produces output one token at a time on the way out.
...
Tokens are the unit everything else is measured in. Cost is per token, providers bill [input tokens](https://fderuiter.github.io/agy-skills/dictionary/input-tokens) and [output tokens](https://fderuiter.github.io/agy-skills/dictionary/output-tokens) separately. Speed is tokens per second, since output is generated one token at a time. And the context window is a fixed number of tokens, so the token count of your files decides how much fits.
