---
layout: default
title: "Non-determinism | AI Coding Dictionary"
...
permalink: /dictionary/non-determinism/
category: dictionary
keywords: ["ai coding", "dictionary", "non-determinism"]
---

# Non-determinism

The same input can produce different output. Run a [model](https://fderuiter.github.io/agy-skills/dictionary/model) twice with identical [context](https://fderuiter.github.io/agy-skills/dictionary/context) and you may get two different answers, sometimes a word, sometimes a completely different approach. Nothing in your code has to change for this to happen.

It's a property of how models generate text, and how [model providers](https://fderuiter.github.io/agy-skills/dictionary/model-provider) serve [requests](https://fderuiter.github.io/agy-skills/dictionary/model-provider-request). During [inference](https://fderuiter.github.io/agy-skills/dictionary/inference), the model produces a probability distribution over possible next [tokens](https://fderuiter.github.io/agy-skills/dictionary/token) and one is sampled from it, usually with some randomness on purpose, since always picking the most likely token produces repetitive, lower-quality text. One differently-sampled token early in a response changes every token after it, which is how a single different word becomes a completely different approach. Provider-side serving adds more variation on top: requests are batched together on shared hardware, and tiny floating-point differences between batches can tip a close call between two tokens. There's no setting you can flip to make it all go away.

Expect a spread of results from an [agent](https://fderuiter.github.io/agy-skills/dictionary/agent) on the same task. Most responses fall within a reasonable bell curve of quality, that's why the non-determinism is tolerable at all, but the tails are real: some days the model will feel sharp; some days it'll feel like it's lost the plot. Same task, different rolls of the dice. This has two practical consequences. Retrying is a legitimate strategy: a failed attempt is one draw from the distribution, and a fresh attempt at the same task may simply land better. And verification matters more than it would with deterministic tools, you can't test an agent's behaviour once and rely on it repeating, so [automated checks](https://fderuiter.github.io/agy-skills/dictionary/automated-check) have to catch the bad draws.
