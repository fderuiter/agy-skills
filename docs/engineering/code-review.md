...
`code-review` reviews the diff between `HEAD` and a fixed point you name (a commit, a branch, a tag, `main`, `HEAD~5`) along two axes. **Standards** asks whether the code follows how this repo writes code. **Spec** asks whether the code does what the originating issue or [spec](https://fderuiter.github.io/agy-skills/dictionary/spec) asked for. Each axis runs in its own [sub-agent](https://fderuiter.github.io/agy-skills/dictionary/subagent) inside a shared worktree (`Workspace: "share"`), returning structured finding envelopes so neither sees the other's reasoning.

The two axes are never merged and never re-ranked. The report ends with a worst issue *per axis* and refuses to name a single winner across them, because a change can pass one axis and fail the other: code that follows every convention while implementing the wrong thing passes Standards and fails Spec; code that does exactly what the [ticket](https://fderuiter.github.io/agy-skills/dictionary/ticket) asked while breaking the repo's conventions does the reverse. A blended verdict lets the passing axis hide the failing one.
...
| Nothing is written yet and you want it written test-first | [tdd](https://fderuiter.github.io/agy-skills/skills-tdd) |
| A whole spec needs building, review included | [implement](https://fderuiter.github.io/agy-skills/skills-implement), which calls this skill itself |
| The whole codebase has drifted, not one diff | [improve-codebase-architecture](https://fderuiter.github.io/agy-skills/skills-improve-codebase-architecture) |
| Something is broken and you do not know why | [diagnosing-bugs](https://fderuiter.github.io/agy-skills/skills-diagnosing-bugs) |
...
Step 1 depends on `docs/agents/issue-tracker.md`, which [setup-agy-skills](https://fderuiter.github.io/agy-skills/skills-setup-agy-skills) writes. Without it the axis still works if you hand it a path. With no spec at all, the Spec sub-agent is skipped and the report says "no spec available" rather than inventing requirements.
...
This is the most reported problem with the skill, and it is not fixed. Antigravity ships its own `/code-review`, which does something different: it hunts bugs in the diff, where this one checks spec compliance and repo standards. Installing this library means one of them wins, and which one wins depends on how you installed. Via plugin discovery, the local customization takes precedence; via a plain skills install, the local file wins and this skill shadows the built-in. One clean answer is to remove Antigravity's built-in skills entirely: a large [context](https://fderuiter.github.io/agy-skills/ai-coding-dictionary/context) saving, and the collision stops mattering. The shadowing itself is arguably an agent [harness](https://fderuiter.github.io/agy-skills/ai-coding-dictionary/harness) bug (a skill author should be free to name a skill anything), so the other answer is to rename the local copy. Editing the frontmatter or renaming the directory gets undone if overwritten; the durable workaround is to fork the skill to a new name and drop `code-review` from the managed set.
...
- [implement](https://fderuiter.github.io/agy-skills/skills-implement) is the closest neighbour: it drives the build and calls this skill as its own closing review before committing.
- [to-spec](https://fderuiter.github.io/agy-skills/skills-to-spec) and [to-tickets](https://fderuiter.github.io/agy-skills/skills-to-tickets) produce the document the Spec axis checks against; a vague spec makes that axis vague.
- [improve-codebase-architecture](https://fderuiter.github.io/agy-skills/skills-improve-codebase-architecture) is the whole-codebase counterpart: this skill only ever looks at one diff.

[ask-fred](https://fderuiter.github.io/agy-skills/skills-ask-fred) routes across the whole set when you are unsure which skill the situation wants.
