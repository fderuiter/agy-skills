...
`to-spec` turns the conversation you have just had into a **[spec](https://fderuiter.github.io/agy-skills/dictionary/spec)**, and publishes it to your issue tracker as a single issue.
...
You invoke this by typing `/to-spec`; the [agent](https://fderuiter.github.io/agy-skills/dictionary/agent) won't reach for it on its own.

Reach for it when the build is too big for one agent [session](https://fderuiter.github.io/agy-skills/dictionary/session) and has to survive being split across several. That is the whole trigger:

| Where you are | What to run |
| --- | --- |
| You haven't decided anything yet | [grill-with-docs](https://fderuiter.github.io/agy-skills/skills-grill-with-docs) first |
| Decided, and the work fits one [context window](https://fderuiter.github.io/agy-skills/dictionary/context-window) | [implement](https://fderuiter.github.io/agy-skills/skills-implement): skip the spec |
| Decided, and the work spans several sessions | `/to-spec`, then [to-tickets](https://fderuiter.github.io/agy-skills/skills-to-tickets) |
| A [wayfinder](https://fderuiter.github.io/agy-skills/skills-wayfinder) map has cleared | `/to-spec #<map_issue>` |

## Prerequisites

`to-spec` publishes the spec as an issue, so [setup-agy-skills](https://fderuiter.github.io/agy-skills/skills-setup-agy-skills) must have configured a tracker and the triage-label vocabulary for this repo first. Either kind works: a real tracker like GitHub, or local markdown files under `.scratch/`, which is supported out of the box.
...
Less well, and this is a known limitation. The template leans hard on user stories, which is the wrong shape for architectural work: you end up writing stories nobody asked for around decisions that are really about interfaces and invariants. Lean on the implementation-decisions and testing-decisions sections instead, and let the durable architectural calls land as ADRs via [grill-with-docs](https://fderuiter.github.io/agy-skills/skills-grill-with-docs) rather than trying to make the spec carry them.
...
Its neighbours upstream are [grill-with-docs](https://fderuiter.github.io/agy-skills/skills-grill-with-docs), which does the deciding this skill only records, and [wayfinder](https://fderuiter.github.io/agy-skills/skills-wayfinder), whose finished map merges onto the chain right here. Downstream, [to-tickets](https://fderuiter.github.io/agy-skills/skills-to-tickets) cuts the spec into tracer-bullet tickets for [implement](https://fderuiter.github.io/agy-skills/skills-implement) to build. When you're unsure which skill or flow fits, [ask-fred](https://fderuiter.github.io/agy-skills/skills-ask-fred) routes you.
