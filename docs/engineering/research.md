...
`research` answers a question by reading the sources that own the answer, then leaves a cited Markdown file in the repo. It works only from **[primary sources](https://fderuiter.github.io/agy-skills/dictionary/primary-source)**: official docs, source code, specs, first-party APIs. It follows every claim back to the source that owns it, so it will not repeat a blog post's account of an API when the API's own docs are reachable.

It does not answer you in the conversation. The output is a file, written where the repo already keeps such notes, with a link on each claim. That is the point: a document you can react to, hand to another agent, or throw away, rather than an answer that vanishes when the [session](https://fderuiter.github.io/agy-skills/dictionary/session) ends.

## When to reach for it

Type `/research`, or the [agent](https://fderuiter.github.io/agy-skills/dictionary/agent) reaches for it automatically when a task turns into reading legwork.
...
| A decision made *with* you, by interview | [grilling](https://fderuiter.github.io/agy-skills/skills-grilling) |
| A durable architecture decision, written into `CONTEXT.md` and ADRs | [grill-with-docs](https://fderuiter.github.io/agy-skills/skills-grill-with-docs) |
| To find out whether an approach works in your codebase | [prototype](https://fderuiter.github.io/agy-skills/skills-prototype) |
| A plan too big to hold in one session | [wayfinder](https://fderuiter.github.io/agy-skills/skills-wayfinder) |

The line between `research` and `grill-with-docs` is the **shelf life of what comes back**. Research produces short-lived assets: what this library's auth mechanism does as of this week. An ADR records a decision you keep. If what you are producing is a decision rather than a fact, you are [grilling](https://fderuiter.github.io/agy-skills/dictionary/grilling), not researching.
...
No. This is an open bug, [issue #530](https://github.com/fderuiter/agy-skills/issues/530). The skill tells its caller to spin up a background agent but does not restrict the agent type, so the agent it spawns is a `general-purpose` one that holds the `Agent` tool and the same instructions, and fires them again. One reporter measured a single research task costing roughly 450k [tokens](https://fderuiter.github.io/agy-skills/dictionary/token) across three overlapping runs, with the duplicate finishing half an hour later entirely out of view. It reproduces outside Antigravity too; the same nesting was confirmed in Antigravity with GPT-5.6-sol. There is no shipped fix. Users have patched their own installed copy with a line telling an agent that is already a [subagent](https://fderuiter.github.io/agy-skills/dictionary/subagent) to do the work itself, which helps but is instruction-level, not structural. Watch your background task list after invoking, and stop the duplicate.
...
No, it now fires them for you. In the unreleased changes since v1.1, a charting session spawns a `/research` subagent per research ticket and burns them down in parallel, capturing findings on a throwaway `research/<name>` branch with a [context pointer](https://fderuiter.github.io/agy-skills/dictionary/context-pointer) from the ticket. Research tickets are the one exception to wayfinder's one-ticket-per-session rule, because they are [AFK](https://fderuiter.github.io/agy-skills/dictionary/afk): nothing waits on you. Two known snags with those branches: the subagent has been seen opening a draft PR from a branch that is never meant to merge ([issue #576](https://github.com/fderuiter/agy-skills/issues/576)), and deleting the branch later breaks the context pointers the tickets hold.
...
A reach-for-it-anytime standalone that feeds the thinking skills rather than sitting in the build chain. Its file is something to take *into* the flow: [grilling](https://fderuiter.github.io/agy-skills/skills-grilling) and [grill-with-docs](https://fderuiter.github.io/agy-skills/skills-grill-with-docs) ask sharper questions when the facts are already on the table, and [to-spec](https://fderuiter.github.io/agy-skills/skills-to-spec) can synthesise against it. [wayfinder](https://fderuiter.github.io/agy-skills/skills-wayfinder) is the one skill that invokes it directly, resolving each research ticket on its map with a `/research` subagent. For the whole map, see [ask-fred](https://fderuiter.github.io/agy-skills/skills-ask-fred).
