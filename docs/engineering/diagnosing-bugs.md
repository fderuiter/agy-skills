...
| A raw bug report from someone else, not yet confirmed or written up | [triage](https://fderuiter.github.io/agy-skills/skills-triage) first |
| Throwaway code to answer a design question, not chase a defect | [prototype](https://fderuiter.github.io/agy-skills/skills-prototype) |
| Building a planned behaviour test-first | [tdd](https://fderuiter.github.io/agy-skills/skills-tdd) |
| No good seam exists to lock the bug down | [improve-codebase-architecture](https://fderuiter.github.io/agy-skills/skills-improve-codebase-architecture): this skill hands off there itself |
...
When it genuinely cannot build one, it is instructed to stop and say so, list what it tried, and ask you for [environment](https://fderuiter.github.io/agy-skills/dictionary/environment) access, a captured artifact, or permission to add temporary instrumentation. It should not proceed to hypothesise anyway.
...
This is the most-reported problem with the skill, and it is real. On GPT-5.6-Sol especially, users report it triggering on a plain description of a problem: "the model triggers the rather formal diagnosing-bugs skill instead. It then goes on to construct a reproduction scenario (often building a mock scenario with limited value) before giving me a response or suggestion. This results in considerable reply delays." Four separate people reported the same shape on [issue #578](https://github.com/fderuiter/agy-skills/issues/578). The accepted fix is to start with a lighter approach and graduate to the heavier one only where the problem warrants it, but that change has not landed. The skill is calibrated against Antigravity's invocation behaviour; a [model](https://fderuiter.github.io/agy-skills/dictionary/model) with a lower activation threshold over-fires it. Until it is graduated, the practical fix is to say what you want ("just answer this, don't diagnose") or to disable model invocation for it in your [harness](https://fderuiter.github.io/agy-skills/dictionary/harness).
...
It might. The skill asks the agent to paste the invocation and its output, and to request artifacts like HAR files, log dumps, and core dumps. None of those are sanitised by instruction. [Issue #674](https://github.com/fderuiter/agy-skills/issues/674) raises exactly this (credentials, tokens, cookies, and personal data riding along into a chat, an issue, or a PR) and proposes a redaction guardrail. It is open and unimplemented. Treat redaction as your job for now, particularly before the output goes anywhere public.
...
`diagnosing-bugs` is a reach-for-it-anytime standalone. You drop into it when something is broken and drop out when the fix and its regression test are in; it holds no state and needs no prior setup. [ask-fred](https://fderuiter.github.io/agy-skills/skills-ask-fred) routes "Something's broken" here.

Two neighbours matter. [improve-codebase-architecture](https://fderuiter.github.io/agy-skills/skills-improve-codebase-architecture) takes the [handoff](https://fderuiter.github.io/agy-skills/dictionary/handoff) when the real finding is that the code has no seam to lock the bug down; the recommendation is made after the fix is in, when there is more information. [triage](https://fderuiter.github.io/agy-skills/skills-triage) sits upstream of it for bugs that arrive as raw reports from other people, and does a shallower version of the same first two phases.
