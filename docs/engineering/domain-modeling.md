...
It is the **active** discipline, not the passive one. Reading `CONTEXT.md` to borrow its vocabulary is a one-line habit any skill can do; this skill is for when you are *changing* the model. That is what makes it interrupt. It writes a resolved term into `CONTEXT.md` at the moment it is resolved, in the middle of the conversation, rather than producing a tidy glossary at the end, because the batched version is a summary of a [session](https://fderuiter.github.io/agy-skills/dictionary/session), and the inline version is the session's actual output.

## When to reach for it

Type `/domain-modeling`, or the agent reaches for it automatically when a task fits. In practice, automatic invocation is the weakest part of the skill: when `grill-with-docs` or `wayfinder` say to load it, [models](https://fderuiter.github.io/agy-skills/dictionary/model) frequently load `grilling` and skip this one. If a [grilling](https://fderuiter.github.io/agy-skills/dictionary/grilling) session runs and `CONTEXT.md` is untouched at the end, that is what happened; invoke it by name alongside the other skill.
...
| The module's *shape* is the problem: where the seam goes, how deep the interface is | [codebase-design](https://fderuiter.github.io/agy-skills/skills-codebase-design) |
| You want the whole plan interrogated before you build | [grill-with-docs](https://fderuiter.github.io/agy-skills/skills-grill-with-docs), which drives this skill underneath |
...
| Never holds | Implementation details, a [spec](https://fderuiter.github.io/agy-skills/dictionary/spec), a scratch pad, general programming concepts | A diary of every choice made this session |
...
This is the most-argued naming question in the whole skill set and it has no settled answer. The case against the current name is good: if it is "a glossary and nothing else", `GLOSSARY.md` says so, and, as one reader put it, "with ai agents everything is [context](https://fderuiter.github.io/agy-skills/dictionary/context)". The case for it is the map: `CONTEXT-MAP.md` pointing at several `CONTEXT.md` files reads naturally in a way `GLOSSARY-MAP.md` does not, and `context` is the standing DDD word for a bounded area of the model. At least one person maintains a local fork purely to rename the file. You can do the same, but every other skill in the set looks for `CONTEXT.md`, so a rename means patching all of them.
...
`domain-modeling` is a **model-invoked reference** that runs *underneath* other skills more often than it runs on its own. [grill-with-docs](https://fderuiter.github.io/agy-skills/skills-grill-with-docs) drives it through a grilling session, [wayfinder](https://fderuiter.github.io/agy-skills/skills-wayfinder) loads it while charting a map, [triage](https://fderuiter.github.io/agy-skills/skills-triage) uses it to keep [tickets](https://fderuiter.github.io/agy-skills/dictionary/ticket) in the project's own words, and [improve-codebase-architecture](https://fderuiter.github.io/agy-skills/skills-improve-codebase-architecture) calls it as decisions crystallise. Its closest sibling is [codebase-design](https://fderuiter.github.io/agy-skills/skills-codebase-design): the two are the vocabulary layer under everything else, this one for the *domain*, that one for the module's *shape*. It is also reachable directly, when you want the discipline without committing to the steps of whatever skill would normally pull it in. When you are unsure which skill fits, [ask-fred](https://fderuiter.github.io/agy-skills/skills-ask-fred) routes you.
