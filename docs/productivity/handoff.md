...
`handoff` compacts the conversation you are in into a **handoff document**: one markdown file, written to your OS's temporary directory rather than into the workspace, that a fresh [agent](https://fderuiter.github.io/agy-skills/dictionary/agent) can read to pick the work up.

What it buys is **portability**, not compression. That makes the skill narrower than it sounds. You need a file only when the work has to *travel*: to a new [harness](https://fderuiter.github.io/agy-skills/dictionary/harness), a new directory, a colleague, or a side task you want to fork off. If nothing is travelling, you do not need a handoff: staying in the [session](https://fderuiter.github.io/agy-skills/dictionary/session), `/clear`, a [subagent](https://fderuiter.github.io/agy-skills/dictionary/subagent) and `/compact` cover the ordinary end-of-phase case, and `/compact` covers it more often than this skill does.
...
| --- | --- |
| Swapping harness (Antigravity → Antigravity) | The new harness cannot see the old [context](https://fderuiter.github.io/agy-skills/dictionary/context) |
...
For anything else (same harness, same directory, you are done [grilling](https://fderuiter.github.io/agy-skills/dictionary/grilling) and moving to implementation), `/compact` is the move. [ask-fred](https://fderuiter.github.io/agy-skills/skills-ask-fred) carries the ordered tree over all five options at a phase boundary.
